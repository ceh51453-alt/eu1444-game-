...
function fallbackArmy(name: string, troops = 1800): ForceSpec {
  return {
    name,
    troops,
    composition: [
...
function enemySpec(state: GameState, playerForceId: string, fallbackTroops: number): { force: ForceSpec; place: string } {
  ...
  if (enemy !== undefined) {
    return {
      force: fallbackArmy(enemy.name, Math.max(80, enemy.troops)),
      place: campaignNode(enemy.nodeId)?.name ?? 'vị trí tranh chấp trên chiến đồ',
    };
  }
  return {
    force: fallbackArmy('Đạo quân đối phương', Math.max(80, Math.round(fallbackTroops * 0.9))),
...
export function createFieldBattle(state: GameState, rng: Rng, turn: number): BattleState {
  ...
  const ours = land === undefined
    ? fallbackArmy('Đạo quân của người chơi')
    : forceSpec(land, name === '' ? 'Chủ soái' : name, faction) ?? fallbackArmy('Đạo quân của người chơi');
